...
from OT.utils import wasserstein_distance_two_gaussians, get_ETTs, run_tt_als, ETT_fits_predict, uv_sample, \
    validate_qq_model, domain_adjust, get_base_dist_quantiles, get_data_pair
...
if __name__ == '__main__':
    ...
    Xq_train, Yq_train = get_data_pair(base_dist=base_dist, target_dist=target_dist, data_batch_size=data_batch_size,
                                       num_batches=num_batches_train, num_p_levels=num_p_levels,
                                       torch_dtype=torch_dtype, torch_device=torch_device,
                                       transformer=train_transformer)
    Xq_test, Yq_test = get_data_pair(base_dist=base_dist, target_dist=target_dist, data_batch_size=data_batch_size,
                                     num_batches=num_batches_test, num_p_levels=num_p_levels,
                                     torch_dtype=torch_dtype, torch_device=torch_device, transformer=test_transformer)
    # Modeling
    model = get_ETTs(D_in=D + 1, D_out=D, rank=4, domain_stripe=[-1, 1], poly_degree=4, device=torch_device)
    run_tt_als(x=Xq_train, y=Yq_train, ETT_fits=model, test_ratio=0.2, tol=1e-6, domain_stripe=[-1, 1],
               max_iter=200, regularization_coeff=float(1e-2))
